...
function preencherTabela(tipo, dados) {
    ...
    dados.forEach(item => {
        ...
        Object.values(item).forEach(valor => {
            ...
            // aqui deve mostar o nome do autor e categoria em vez do id
            if (tipo === 'livros') {
                const autor = autores.find(a => a.id_autor === item.autor_id);
                const categoria = categorias.find(c => c.id_categor === item.categoria_id);
                td.textContent = `${item.titulo} - ${autor ? autor.nome : 'Desconhecido'} - ${categoria ? categoria.nome : 'Desconhecida'}`;
            }
            tr.appendChild(td);
        });
