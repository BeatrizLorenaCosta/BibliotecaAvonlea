...
function carregarSelects() {
    fetch('http://localhost:3000/api/autores')
        ...
        .then(data => {
            ...
            data.forEach(autor => {
                ...
                option.value = autor.id_autor; // CORRIGIDO
                option.textContent = autor.nome_autor; // CORRIGIDO
    ...
    fetch('http://localhost:3000/api/categorias')
        ...
        .then(data => {
            ...
            data.forEach(cat => {
                const option = document.createElement('option');
                option.value = cat.id_categoria; // CORRIGIDO
                option.textContent = cat.nome_categoria; // CORRIGIDO
...
function preencherTabela(tipo, dados) {
    ...
    dados.forEach(item => {
        ...
        if (tipo === 'livros') {
            ...
            console.log("nome do autor:", autor ? autor.nome_autor : 'Desconhecido');
            console.log("nome da categoria:", categoria ? categoria.nome_categoria : 'Desconhecida');
