...
document.addEventListener('DOMContentLoaded', async () => {
    // Carregar todos os dados principais antes de preencher tabelas
    await Promise.all([
    ...
    sections.forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            await carregarDados(btn.dataset.section);
        });
    });
});
...
function carregarDados(tipo) {
    return fetch(`http://localhost:3000/api/${tipo}`)
        .then(res => res.json())
        .then(dados => {
            window[tipo] = dados;
            ...
            // Atualizar tabelas dependentes se já tiverem dados
            if (dependencias[tipo]) {
                dependencias[tipo].forEach(dep => {
                    if (window[dep] && !document.getElementById(dep).classList.contains('hidden')) {
