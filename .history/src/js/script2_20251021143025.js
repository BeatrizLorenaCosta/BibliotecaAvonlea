...
// Simulação (vai depois vir do backend)
let contas = [
  { id_conta: 1, utilizador_id: 1, email: "maria@escola.pt", senha: "12345", tipo: "professor" },
  { id_conta: 2, utilizador_id: 2, email: "joao@aluno.pt", senha: "12345", tipo: "aluno" },
  { id_conta: 3, utilizador_id: 3, email: "admin@avonlea.pt", senha: "admin", tipo: "outro" }
];
...
function fazerLogin() {
    ...
    if (conta) {
        ...
        alert(`Bem-vindo(a), ${conta.tipo === 'outro' ? 'Administrador' : conta.tipo}!`);
        aplicarPermissoes(conta);
    } else {
        erro.style.display = "block";
    }
}

function aplicarPermissoes(conta) {
    ...
    document.getElementById("login").textContent = "🚪 Sair";
    document.getElementById("login").onclick = logout;

    // 🔒 Controlar permissões
    if (conta.tipo === "aluno" || conta.tipo === "professor") {
        // Só podem ver avaliações e adicionar as suas
