...
function fazerLogin() {
    ...
    // 1️⃣ procurar o utilizador pelo email
    ...
    // 2️⃣ procurar a conta associada
    const conta = contas.find(c => c.utilizador_id === utilizador.id_utilizador && c.senha === senha);
    ...
    const utilizadorLogado = { ...utilizador, id_conta: conta.id_conta };
    localStorage.setItem("contaLogada", JSON.stringify(utilizadorLogado));
    erro.style.display = "none";

    alert(`Bem-vindo(a), ${utilizador.nome_utilizador}!`);
    aplicarPermissoes(utilizadorLogado);
}
