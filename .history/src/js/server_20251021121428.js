...
app.get('/api/livros', (req, res) => {
    db.query('SELECT l.id_li l.id_livro, l.titulo, a.nome_autor, c.nome_categoria, l.ano, l.disponivel FROM livros l JOIN autores a ON l.autor_id = a.id_autor JOIN categorias c ON l.categoria_id = c.id_categoria', (err, results) => {
...
app.get('/api/emprestimos', (req, res) => {
    db.query('SELECT l.titulo, u.nome_utilizador, e.data_emprestimo, e.data_devolucao FROM emprestimos e JOIN livros l ON e.livro_id = l.id_livro JOIN utilizadores u ON e.utilizador_id = u.id_utilizador', (err, results) => {
        if (err) return res.status(500).json({erro: err});
        res.json(results);
    });
});
