...
app.get('/api/emprestimos', (req, res) => {
    db.query('SELECT e.id_emp l.titulo, u.nome_utilizador, e.data_emprestimo, e.data_devolucao FROM emprestimos e JOIN livros l ON e.livro_id = l.id_livro JOIN utilizadores u ON e.utilizador_id = u.id_utilizador', (err, results) => {
        if (err) return res.status(500).json({erro: err});
        res.json(results);
    });
});
